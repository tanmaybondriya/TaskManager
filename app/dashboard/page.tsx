...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function bootstrap() {
      try {
        ...
        const projectResponse = await apiFetch<{ projects: Project[] }>(
          "/projects",
        );
  ...
  const loadIssues = useCallback(
    async (projectId = activeProjectId) => {
      ...
      const response = await apiFetch<{ issues: Issue[] }>(
  ...
  useEffect(() => {
    if (activeProjectId) {
      loadIssues(activeProjectId).catch((err) =>
  ...
  const displayedItems = useMemo(() => {
    ...
    return merged.filter((item) => {
      ...
      if (
        ...
        (isDefaultTask(item) || item.assignee !== user?.id)
      )
        return false;
      if (
        search &&
  ...
  async function createIssue(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await apiFetch<{ issue: Issue }>(
  ...
  async function moveItem(item: BoardItem, status: Status) {
    ...
    setIssues((current) =>
      current.map((issue) =>
        issue._id === item._id ? { ...issue, status } : issue,
      ),
    );
    ...
    setIssues((current) =>
      current.map((issue) => (issue._id === item._id ? response.issue : issue)),
    );
  }
  ...
  async function addComment(issueId: string, body: string) {
    ...
    setIssues((current) =>
      current.map((issue) => (issue._id === issueId ? response.issue : issue)),
    );
    setSelectedItem(response.issue);
  }
  ...
  return (
    <main className="min-h-screen md:flex">
      ...
      <section className="flex min-w-0 flex-1 flex-col">
        ...
        {error && (
          <p className="mx-4 mt-4 rounded-md bg-red-50 px-3 py-2 text-sm text-red-700 md:mx-6">
            {error}
          </p>
        )}
            ...
            <section className="rounded-lg border border-slate-200 bg-white p-4">
              ...
              <div className="space-y-3">
                ...
                <form className="flex gap-2" onSubmit={applySearch}>
                  ...
                  />
                  <button
                    className="rounded-md bg-slate-900 px-3 text-white"
                    type="submit"
                  >
                    <Search size={16} />
                  </button>
