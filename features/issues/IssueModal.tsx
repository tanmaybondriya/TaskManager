...
import type { BoardItem, Priority, Status } from "@/types";
...
export function IssueModal({ item, onClose, onSave, onDelete, onComment }: Props) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4">
      <section className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-lg bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
          <div>
            <p className="text-xs font-semibold uppercase text-slate-500">{isDefault ? "Default task" : "Issue"}</p>
            <h2 className="text-lg font-semibold text-slate-950">{values.title}</h2>
          </div>
        ...
        <div className="grid gap-5 p-5 md:grid-cols-[1fr_220px]">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  {activeItem.comments?.map((entry) => (
                    <div className="rounded-md border border-slate-200 p-3 text-sm" key={entry._id}>
                      <p className="font-semibold text-slate-800">{entry.authorName}</p>
                      <p className="mt-1 text-slate-700">{entry.body}</p>
                    </div>
                  ))}
                  {!activeItem.comments?.length && <p className="text-sm text-slate-500">No comments yet.</p>}
                </div>
              </div>
            )}
          </div>
          ...
          <aside className="space-y-4">
            ...
            {!isDefault && activeItem.activity?.length ? (
              <div>
                <h3 className="mb-2 text-sm font-semibold text-slate-800">Activity</h3>
                <div className="space-y-2">
                  {activeItem.activity.slice().reverse().map((entry) => (
                    <p className="rounded-md bg-slate-50 px-3 py-2 text-xs text-slate-600" key={entry._id}>
